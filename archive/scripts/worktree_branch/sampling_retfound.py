"""
scripts/sampling_retfound.py

Generate images from a trained JiT-RCDM checkpoint conditioned on RETFound
representations.

Origin / changes vs scripts/sampling.py (DinoV3 ViT-S/16)
-------------------------------------------------------------
  Copy of sampling.py with only the conditioning encoder swapped:

    1. Encoder: DinoV3 ViT-S/16 (CLS token, 384-dim)
                → RETFound MAE ViT-Large/16 (CLS token, 1024-dim).

    2. --encoder_ckpt default: checkpoints/dinov3_vits16_tmp
                → checkpoints/retfound/RETFound_mae_natureCFP.pth

    3. CLS extraction: out.last_hidden_state[:, 0, :] (HF API)
                → encoder.forward_features(x)[:, 0, :] (timm API).

  Everything else (model loading via model_cfg, EMA handling, Heun ODE
  sampling, output grid, W&B logging) is unchanged. model_cfg["h_dim"] will
  read 1024 for checkpoints trained on RETFound representations.

For each conditioning image supplied via --cond_images, the script:
  1. Loads the image and encodes it with RETFound to get h (1024-dim CLS token).
...
def load_model(checkpoint_path: str, device: torch.device):
    ...
    If the checkpoint contains an EMA shadow dict, apply EMA weights instead
    of the raw model weights — EMA consistently produces better sample
    quality (JiT paper Tab. 9).
    """
...
def main():
    ...
    parser.add_argument("--encoder_ckpt", type=str, default=RETFOUND_CHECKPOINT,
                        help="Path to the local RETFound MAE ViT-L/16 .pth checkpoint")
    ...
    # Always encode at 224 px — RETFound ViT-L/16 has fixed pos-embeds for a
    # 14x14 patch grid (224/16), same constraint as DinoV3.
    ...
    for img_path in args.cond_images:
        ...
        # Encode conditioning image → h (1024-dim CLS token)
        img_pil = Image.open(img_path).convert("RGB")
        x_enc   = enc_transform(img_pil).unsqueeze(0).to(device)
        with torch.no_grad():
            feats = encoder.forward_features(x_enc)
        h_single = feats[:, 0, :]                                # (1, 1024)
        h        = h_single.expand(args.n_samples, -1)           # (n_samples, 1024)
