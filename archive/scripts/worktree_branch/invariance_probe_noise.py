"""
scripts/invariance_probe_noise.py

Variant of invariance_probe.py that does NOT fix the starting noise.

This is a copy of invariance_probe.py with one deliberate change: instead of
drawing a single seeded noise tensor and reusing it across every variant, each
variant draws FRESH noise via torch.randn(...) exactly like scripts/sampling.py.

Consequence: differences between rows now mix two factors — the conditioning h
(from the augmentation) AND the random noise draw. Use this when you want to see
the model's natural sample diversity per variant rather than a controlled,
noise-matched comparison. For the controlled invariance test (noise held fixed
so only h varies), use invariance_probe.py instead.

For every grade folder in test_images_invariance/<grade>/ the script:

  1. Encodes the original + augmented images with DinoV3 -> CLS token h (384).
  ...
  3. Generates --n_samples images from each variant's h, drawing fresh random
     noise per variant (unseeded, as in sampling.py).
  4. Saves one comparison grid per grade: each row is a variant
     [labelled input | generated samples].
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="invariance_noise_samples")
    ...
    parser.add_argument("--wandb_run_name", type=str, default="invariance-probe-noise")
    ...
    if use_wandb:
        run = wandb.init(
            ...
            config={
                ...
                "noise": "fresh-per-variant (unseeded)",
                "device": str(device),
    ...
    for grade_dir in grades:
        ...
        for v in variants:
            ...
            # 3) generate n_samples from this variant's h with FRESH random noise
            #    (unseeded, exactly as scripts/sampling.py draws it)
        ...
        if use_wandb:
            wandb_images.append(wandb.Image(
                ...
                caption=(f"{grade} | rows: {', '.join(variants)} | "
                         ...
                         f"generated (fresh noise, cfg={args.cfg_scale})"),
            ))
